...
Versión 2.0 - Integrado con AppPaths
"""
...
class ThemeManager:
    ...
    def __init__(self, themes_dir: Optional[Path] = None):
        ...
        Args:
            themes_dir: Directorio donde se encuentran los archivos JSON de temas.
                       Si es None, usa AppPaths.get_themes_dir()
        """
        if themes_dir is None:
            # Usar AppPaths para compatibilidad con PyInstaller
            self.themes_dir = AppPaths.get_themes_dir()
        else:
            self.themes_dir = Path(themes_dir)
        
        self.current_theme: str = "light"
        self.themes: Dict[str, Dict[str, Any]] = {}
        self.theme_data: Dict[str, Any] = {}  # Almacena el tema actual cargado
        self._load_themes()
    ...
    def _load_themes(self) -> None:
        ...
        if self.current_theme in self.themes:
            self.theme_data = self.themes[self.current_theme]
    ...
    @staticmethod
    def get_light_messagebox_style() -> str:
        """
        Retorna un stylesheet fijo en tema claro para QMessageBox.
        
        Este método asegura que todas las ventanas emergentes (QMessageBox)
        sean legibles independientemente del tema activo de la aplicación.
        
        Returns:
            String con el stylesheet CSS para QMessageBox
        """
        return """
            QMessageBox {
